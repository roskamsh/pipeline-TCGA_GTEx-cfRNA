...
newBaseline <- gsub("_GTEx","",baseline_GTEx)
newTarget <- gsub("_TCGA","",target)
...
md <- filter(md, Type == baseline_TCGA | Type == baseline_GTEx | Type == target)
...
# Map the two controls to a new column in colData 
controlName <- gsub("_GTEx","",baseline_GTEx)
md$Group <- ifelse(md$Type == target, gsub("_TCGA","",target), controlName)
...
saveRDS(dds, file=paste0(outDir, "/", contrast, "-dds.rds"))

res <- results(dds, contrast=c("Group", newTarget, newBaseline),  independentFiltering = FALSE, cooksCutoff = Inf)
# shrink fold changes for lowly expressed genes
res <- lfcShrink(dds, contrast=c("Group", newTarget, newBaseline), res=res)
